...
function getDomainColor(domain) {
  const entry = watchlist.find(function (e) { return e.domain === domain })
...
function renderStats() {
  ...
  if (stats.topDomain) {
    ...
    document.getElementById("statTopSiteLabel").textContent = "最多访问 " + stats.topDomainCount + "次"
  } else {
  ...
  container.innerHTML = watchlist
    .map(function (entry, idx) {
      return (
        ...
        '<div class="wd-dot" style="background:' + (entry.color || "#5b8dee") + '"></div>' +
        '<div class="wd-domain">' + escapeHtml(entry.domain) + "</div>" +
        '<div class="wd-label">' + escapeHtml(entry.label || "") + "</div>" +
        '<div class="wd-count">' + (counts[entry.domain] || 0) + " 条</div>" +
        '<button class="wd-remove" data-idx="' + idx + '">×</button>' +
        "</div>"
      )
    })
    .join("")
...
function getFilteredRecords() {
  var filtered = activeFilter === "all"
    ? records
    : records.filter(function (r) { return r.matchedRule === activeFilter })
  if (searchQuery) {
...
function renderRecords() {
  ...
  if (filtered.length === 0) {
    container.innerHTML =
      ...
      '<div class="empty-text">' + (searchQuery ? "无匹配结果" : "暂无记录") + "</div>" +
      "</div>"
    if (searchQuery) {
  ...
  for (var g = 0; g < groups.length; g++) {
    ...
    for (var i = 0; i < group.items.length; i++) {
      ...
      var pinBtn =
        '<span class="rec-pin' + (pinned ? " on" : "") + '" data-action="rec-pin" data-id="' + r.id + '">' +
        (pinned ? "★" : "☆") +
        "</span>"
      var scoreHtml = pinned
        ? '<div class="rec-score">' +
          '<button class="rec-sc-btn" data-action="rec-score-down" data-id="' + r.id + '">−</button>' +
          '<span class="rec-sc-val">' + (score ?? 50) + "</span>" +
          '<button class="rec-sc-btn" data-action="rec-score-up" data-id="' + r.id + '">+</button>' +
          "</div>"
        : ""
      html +=
        ...
        '<div class="rec-meta">' +
        '<span class="rec-domain" style="color:' + color + '">' + escapeHtml(r.matchedRule) + "</span>" +
  ...
  if (!loadedAll && !searchQuery) {
    html +=
      ...
      '<button id="btnLoadMore">加载更多 (' + records.length + " / " + totalRecords + ")</button>" +
      "</div>"
  }
...
function renderFilterBar() {
  ...
  domains.forEach(function (d) {
    ...
    chips.push(
      '<div class="filter-chip' + (active ? " active" : "") + '" data-domain="' + d + '" style="' +
        (active
          ? "background:" + color + ";border-color:" + color
          : "border-color:" + color + "40") +
        '">' + d + ' <span style="opacity:0.7">' + counts[d] + "</span></div>"
    )
  })
...
document.getElementById("watchlistToggle").addEventListener("click", function (e) {
...
document.getElementById("watchlist").addEventListener("click", function (e) {
  ...
  if (btn) { removeEntry(parseInt(btn.dataset.idx)); return }
  ...
  if (scoreUp || scoreDown) {
    ...
    var rec = records.find(function (r) { return r.id === rid })
    if (rec && rec.pinned) {
      var cur = rec.score ?? 50
      var newScore = Math.max(0, Math.min(100, cur + (scoreUp ? 5 : -5)))
      var idx = records.findIndex(function (r) { return r.id === rid })
...
// Filter chips
document.getElementById("filterBar").addEventListener("click", async function (e) {
...
// Record click
document.getElementById("recordsContainer").addEventListener("click", function (e) {
...
document.getElementById("enabledToggle").addEventListener("change", async function (e) {
...
// Input keydown
document.getElementById("inputDomain").addEventListener("keydown", function (e) {
...
async function addEntry() {
  var domain = document.getElementById("inputDomain").value.trim().toLowerCase()
  ...
  if (watchlist.find(function (e) { return e.domain === domain })) {
    showToast("该域名已存在", "error")
    return
  }

  watchlist.push({ domain: domain, label: label, color: color })
  await window.electronAPI.addToWatchlist({ domain: domain, label: label, color: color })
...
window.electronAPI.onUpdate(function (data) {
  if (data.type === "recordAdded") {
    ...
    if (data.record) {
      var idx = records.findIndex(function (r) { return r.id === data.record.id })
      if (idx !== -1) {
