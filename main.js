...
let watchlist = [
  { domain: "bilibili.com", label: "B站", color: "#fb7299" },
]
let enabled = true
let db
let saveTimer = null
...
async function initDatabase() {
  ...
  db.run(`CREATE INDEX IF NOT EXISTS idx_records_dedup ON records(url, tabId, timestamp)`)
  try { db.run("ALTER TABLE records ADD COLUMN pinned INTEGER DEFAULT 0") } catch (e) {}
  try { db.run("ALTER TABLE records ADD COLUMN score INTEGER DEFAULT NULL") } catch (e) {}
...
function loadSettings() {
  ...
  if (row) {
    ...
    dbRun("INSERT INTO settings (key, value) VALUES (?, ?)", ["enabled", "true"])
  }
}
...
function handleExtensionMessage(ws, msg) {
  switch (msg.type) {
    ...
    case "getStats": {
      ...
      ws.send(
        JSON.stringify({ type: "stats", total, stats, enabled }),
      )
      break
    }
    ...
    case "recordUpdated": {
      if (msg.record) {
        dbRun(
          "UPDATE records SET pinned = ?, score = ? WHERE id = ?",
          [msg.record.pinned ? 1 : 0, msg.record.score, msg.record.id],
        )
    ...
    case "exportData": {
      ...
      ws.send(
        JSON.stringify({ type: "exportData", watchlist, records: rows }),
      )
      break
    }
  }
...
function saveWindowBounds() {
  ...
  dbRun(
    "INSERT OR REPLACE INTO settings (key, value) VALUES ('window.x', ?)",
    [String(bounds.x)],
  )
  dbRun(
    "INSERT OR REPLACE INTO settings (key, value) VALUES ('window.y', ?)",
    [String(bounds.y)],
  )
  dbRun(
...
ipcMain.handle("add-watchlist", (_, entry) => {
  if (!watchlist.find((e) => e.domain === entry.domain)) {
    ...
    dbRun(
      "INSERT INTO watchlist (domain, label, color) VALUES (?, ?, ?)",
      [entry.domain, entry.label, entry.color],
    )
    broadcastToExtensions({ type: "watchlistUpdated", watchlist })
    return true
  }
